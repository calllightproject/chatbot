...
def get_ai_response(question, context):
    ...
    for p in paragraphs:
        p_lower = p.lower()
        score = 0
        # Check for matching keywords from the question in the paragraph
        for word in question_words:
            if word in p_lower:
                score += 1
        
        # A simple check for the main topic of the paragraph
        if p.split(':')[0].lower() in question_lower:
            score += 5 # Heavily weight a match in the title
    ...
    # Rule 4: If no good answer is found, escalate to the nurse for safety
